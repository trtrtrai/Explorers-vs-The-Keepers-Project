...
namespace Models.Generals
{
    ...
    public class Purified : MonoBehaviour
    {
        ...
        private void ActiveSkill()
        {
            ...
            for (int i = 0; i < size; i++)
            {
                if (hit[i].transform.parent.TryGetComponent(out Character character))
                {
                    ...
                    {
                        character.Healing(Mathf.RoundToInt(quantity / 2f), false, false);
                    }
                    else
                    {
                        character.Healing(quantity, false, false);
                    }
                    //Debug.Log("The Forest heal " + quantity + " for " + character.name);
                }
            }
